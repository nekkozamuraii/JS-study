...
function Tile(x, y) {
  ...
  this.move = function (px, py, color) {
    ...
    this.count = 20; // 座標計算用カウンタ（20カウンタかけて移動）
...
function tick() {
  ...
  if (moves.length > 0) {
    // 移動中のタイル(moves)がある場合に、それらのタイルを移動
...
function setColor(x, y, c) {
  // 仮に(x,y)座標に色cを設定した時、3つ同じ色が並んでいるか否かを返す
  let flag = true;
  if (1 < x) {
    // 左方向に3つ連鎖していないか
...
function fall() {
  ...
  iterate(function (x, y, t) {
    t.remove = false; // 全てのタイルでremoveフラグをリセット
  });
}
...
function repaint() {
  ...
  // スコア
  ctx.fillStyle = "rgba(200, 133, 30, 50)";
